...
CREATE PROCEDURE `FDQA_All_Values` (
  ...
  out `o_tinyblob` tinyblob,
  out `o_blob` blob,
  out `o_mediumblob` mediumblob,
  out `o_longblob` longblob,
  out `o_tinytext` tinytext,
  out `o_text` text,
  out `o_mediumtext` mediumtext,
  out `o_longtext` longtext,
  out `o_binary` binary(100),
  out `o_set` set('a', 'b', 'c'),
  out `o_varbinary` varbinary(100))
...
BEGIN
  ...
  set `o_tinyblob` = 'klmno';
  set `o_blob` = 'abcde1';
  set `o_mediumblob` = 'ABCDE';
  set `o_longblob` = 'fghij';
  set `o_tinytext` = 'abcde1';
  set `o_text` = 'ABCDE';
  set `o_mediumtext` = 'vuwxy1';
  set `o_longtext` = 'fghij';
  set `o_binary` = 'fghij';
  set `o_set` = 'b';
  set `o_varbinary` = 'fghij';
END;
GO

DROP PROCEDURE `FDQA_All`
GO

CREATE PROCEDURE `FDQA_All`(IN `in_1` int, IN in_2 int, 
