...
begin
  ...
  TAmazonPopulator(FPopulator).AddMessage(amQueueName.Text, MsgVal);
end;
...
begin
  ...
  TAmazonPopulator(FPopulator).RemoveQueuePermissions(amQueueName.Text);
end;
...
begin
  ...
  if MessageDlg('As XML?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
    TAmazonPopulator(FPopulator).GetQueuePropertiesXML(amQueueName.Text)
  else
    TAmazonPopulator(FPopulator).GetQueueProperties(amQueueName.Text);
end;
...
begin
  ...
  TAmazonPopulator(FPopulator).SetQueueProperty(amQueueName.Text);
end;
