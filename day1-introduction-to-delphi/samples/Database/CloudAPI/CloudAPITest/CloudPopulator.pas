...
type
  ...
  private
    ...
    function GetAmazonMessage(const QueueURL: String): TCloudQueueMessage;
  protected
  ...
  public
    ...
    procedure GetQueuePropertiesXML(const QueueName: String);
    procedure GetQueueProperties(const QueueName: String);
    procedure SetQueueProperty(const QueueName: String);
    procedure AddQueuePermissions(const QueueName: String);
    procedure RemoveQueuePermissions(const QueueName: String);
...
function TAmazonPopulator.GetAmazonMessage(const QueueURL: String): TCloudQueueMessage;
var
...
begin
  ...
  try
    ...
    ResponseList := FQueueService.GetMessages(QueueURL, 1, 30, ResponseInfo);

    if (ResponseList <> nil) and (ResponseList.Count > 0) then
      Result := ResponseList.Last;
  finally
    ResponseList.Free;
    ResponseInfo.Free;
  end;
...
procedure TAmazonPopulator.GetQueuePropertiesXML(const QueueName: String);
var
  Response: String;
  QueueURL: String;
begin
  try
    ...
    if QueueURL <> EmptyStr then
    begin
      Response := FQueueService.GetQueuePropertiesXML(QueueURL);
      Log(#13#10 + FormatXMLData(Response), TCloudLoggerType.cltInfo, TCloudConnectionType.cctAmazon);
    end else
...
procedure TAmazonPopulator.GetQueueProperties(const QueueName: String);
var
  ResponseList: TStrings;
  QueueURL: String;
begin
  ...
  try
    try
      ...
      if QueueURL <> EmptyStr then
      begin
        ResponseList := FQueueService.GetQueueProperties(QueueURL, [aqaAll]);
        Log(#13#10 + ResponseList.Text, TCloudLoggerType.cltInfo, TCloudConnectionType.cctAmazon)
      end else
        Log('Error trying to get queue ' + QueueName + ' properties. Queue does not exists', TCloudLoggerType.cltError, TCloudConnectionType.cctAmazon)
    except
...
procedure TAmazonPopulator.SetQueueProperty(const QueueName: String);
var
  QueueURL: String;
begin
  try
    ...
    if QueueURL <> EmptyStr then
    begin
      if FQueueService.SetQueueProperty(QueueURL, 'MaximumMessageSize', '32768') then
  ...
  try
    try
      ...
      begin
        ResponseList := FQueueService.GetQueueProperties(QueueURL, aqaAll);
        // Get account ID
        accountID := SplitString(ResponseList.Values['QueueArn'], ':')[4];

        Perm1 := TAmazonQueuePermission.Create(accountID, aqacSendMessage);
        Perm2 := TAmazonQueuePermission.Create(accountID, aqacAll);

        if FQueueService.AddQueuePermissions(QueueURL, 'testpermid', [Perm1, Perm2]) then
...
procedure TAmazonPopulator.RemoveQueuePermissions(const QueueName: String);
var
  QueueURL: String;
begin
  try
    ...
    if QueueURL <> EmptyStr then
    begin
      if FQueueService.RemoveQueuePermissions(QueueURL, 'testpermid') then
...
procedure TAmazonPopulator.AddMessage(const QueueName, Msg: String);
var
  QueueURL: String;
  MessageId: String;
begin
  try
    ...
    if QueueURL <> EmptyStr then
    begin
      if FQueueService.AddMessage(QueueURL, Msg, MessageID) then
...
begin
  try
    ...
    if QueueURL <> EmptyStr then
    begin
      Response := FQueueService.GetMessagesXML(QueueURL, FQueueService.GetMaxMessageReturnCount, 10);
      Log(#13#10 + FormatXMLData(Response), TCloudLoggerType.cltInfo, TCloudConnectionType.cctAmazon);
    end else
...
begin
  ...
  try
    try
      ...
      if QueueURL <> EmptyStr then
      begin
        ResponseList := FQueueService.GetMessages(QueueURL, FQueueService.GetMaxMessageReturnCount, 10);
...
begin
  try
    ...
    if QueueURL <> EmptyStr then
    begin
      Response := FQueueService.GetMessagesXML(QueueURL, FQueueService.GetMaxMessageReturnCount, 0);
      Log(#13#10 + FormatXMLData(Response), TCloudLoggerType.cltInfo, TCloudConnectionType.cctAmazon);
    end else
      Log('Error trying to get messages', TCloudLoggerType.cltError, TCloudConnectionType.cctAmazon);
  except
...
begin
  ...
  try
    try
      ...
      if QueueURL <> EmptyStr then
      begin
        ResponseList := FQueueService.PeekMessages(QueueURL, FQueueService.GetMaxMessageReturnCount);
...
//      if FStorageService.CreateBucket(BucketName, amzbaNotSpecified, amzrUSWest1, ResponseInfo) then
      if FStorageService.CreateBucket(BucketName, amzbaNotSpecified, amzrNotSpecified, ResponseInfo) then
...
begin
  ...
  try
    try
      ...
      if FStorageService.SetBucketPolicy(BucketName, Policy, ResponseInfo) then
...
begin
  ...
  try
    try
      ...
      if FStorageService.SetBucketACL(BucketName, ACP, ResponseInfo) then
...
begin
  ...
  try
    try
      ...
      ResponseInfo := TCloudResponseInfo.Create;
      if FStorageService.SetBucketNotification(BucketName, NotificationList, ResponseInfo) then
...
begin
  ...
  try
    try
      ...
      if FBlobService.SetContainerACL(ContainerName, aclList, TBlobPublicAccess.bpaBlob, ResponseInfo) then
